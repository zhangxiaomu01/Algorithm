...
public:
    bool isBipartite(vector<vector<int>>& graph) {
        int n = graph.size();
        // We use this array to color the nodes. If two adjacent nodes have the same
        // color, then we know the graph is not bipartite.
        vector<int> color(n, -1);
        queue<int> Q;
        // We need to check each node given the graph may be a forest!
        for (int i = 0; i < n; ++i) {
            if (color[i] == -1) {
                Q.push(i);
                color[i] = 1;
                while(!Q.empty()) {
                    ...
                    for (int v : graph[next]) {
                        if (color[v] == -1) {
                            color[v] = ((color[next] == 1) ? 2 : 1);
                            Q.push(v);
                        }
                        else {
                            if (color[v] == color[next]) return false;
                        }
                    }
                }
            }
...
// We could also do DFS with the similar idea. Omit now.
