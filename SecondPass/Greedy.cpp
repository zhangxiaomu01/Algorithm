...
private:
    ...
    // 0 - no coverage
    // 1 - coverage
    // 2 - has camera in-place
    // The optimal solution must be that leaf node will have no camera installed.
    int addCamera(TreeNode* node) {
        // The null node will always have 'camera', given we will use this info to 
        // never add any camera to leaf nodes.
        if (!node) return 1;

        int left = addCamera(node->left);
        int right = addCamera(node->right);

        // If left & right have coverage, then we return 0 to indicate this node has no coverage;
