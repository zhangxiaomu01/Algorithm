...
public:
    int countSubstrings(string s) {
        int n = s.size();
        // dp[i][j] means whether the s[i..j] is the palindrome. If it is, then we only 
        // need to check s[i-1] == s[j+1]. Please note that typically I like to initialize
        // dp to be n+1 size, for this problem, using n size is easier, given i..j are both
        // inclusive!
        ...
        for (int i = n-1; i >= 0; --i) {
            for (int j = i; j < n; ++j) {
                if (s[i] == s[j]) {
                    // if s[i] == s[j], then we know that 'a' or 'aa' are both valid!
                    // j-i <= 1 check is necessary for avoiding above corner cases.
                    if (j - i <= 1 || dp[i+1][j-1]) {
                        dp[i][j] = true; 
                        res++;
                    } 
