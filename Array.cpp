...
/* A very classic problem for how to handling the non-decreasing stack.
Solution: https://leetcode.com/problems/maximum-subarray-min-product/discuss/1198896/O(n)-Monostack-with-picture
Like this question.
...
public:
    int maxSumMinProduct(vector<int>& nums) {
        ...
        int len = nums.size();
        long res = 0;
        vector<int> st; // Stores the index of the maximum elements
        ...
        for(int i = 0; i <= len; ++i) {
            while(!st.empty() && (i == len || nums[st.back()] > nums[i])) {
                ...
                int j = st.back();
                st.pop_back();
                // We need to add + 1 here because st.back() will be the smaller than nums[j]
                // In this step, we are examing the subsequence which includes nums[j] as the smallest
                res = max(res, long(nums[j] * (sums[i] - sums[st.empty() ? 0 : st.back() + 1])));
            }
            st.push_back(i);
        }
        return res % MOD;
...
// Another more intuitive but slower implementation:
// https://leetcode.com/problems/maximum-subarray-min-product/discuss/1198708/C%2B%2B-simple-solution-using-stacks-oror-Explained!!
/* A very classic problem for how to handling the non-decreasing stack.
Solution: https://leetcode.com/problems/maximum-subarray-min-product/discuss/1198896/O(n)-Monostack-with-picture
Like this question.
Very important question!
